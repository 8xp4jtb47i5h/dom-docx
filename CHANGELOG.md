...
- **Guard coverage for the three fixes above.** `guard:border-width` gained checks for the explicit-zero-vs-generic-border-fallback fix and the zero/non-px-unit fixes; `guard:table-width-units` gained a check for the width-regex boundary fix.
