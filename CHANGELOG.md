...
- **`vertical-align: super/sub` and `<sup>`/`<sub>` → superscript/subscript runs.** CSS `vertical-align:super` / `vertical-align:sub` and HTML `<sup>`/`<sub>` on inline spans emit `w:vertAlign superscript` / `subscript`. Suite case: `vertical-align-super-sub`. (Thanks to Alexander Wilms for a community patch.)
