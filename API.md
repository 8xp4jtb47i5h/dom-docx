...
| `tableOfContents` | `boolean \| { title?, headingRange?, hyperlink?, pageBreakAfter? }` | — | Inserts a Table of Contents field at the top, built from the document's `h1`–`h6` (mapped to Word Heading 1–6). Heading titles are **cached** into the field so the TOC renders immediately in every viewer (LibreOffice, Google Docs, PDF/preview); page numbers are computed by the word processor on field update. The field is emitted "dirty" with `updateFields`, so Word fills page numbers on open — elsewhere, right-click → Update Field (or Tools → Update). `headingRange` defaults to `"1-3"`; `hyperlink` to `true`. |
